...
export default function SocialShareModal({ scholarship, open, onClose }) {
  ...
  const handlePublishAll = async () => {
    ...
    setStatus(null);
    try {
      const data = await scholarshipsApi.share(scholarship._id);
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-4 p-6 border-b border-white/8">
              <div>
                <div className="flex items-center gap-2 text-[#2FA084] mb-1">
                  <Share2 className="w-4 h-4" />
                  <span className="text-xs font-bold uppercase tracking-wider">Publish to Social</span>
                </div>
                <h2 className="text-lg font-bold text-white leading-snug">{scholarship.title}</h2>
                <p className="text-xs text-white/40 mt-1">Share live to all platforms in real time</p>
              </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <button
                  ...
                  disabled={publishing}
                  className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-[#2FA084] text-white text-sm font-bold hover:bg-[#3CD1AD] transition disabled:opacity-50"
                >
