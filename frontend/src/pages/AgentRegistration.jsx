...
import { CheckCircle2, Upload, FileText, X, AlertCircle, User, Briefcase, Building2, ShieldCheck, ChevronRight } from 'lucide-react';
...
export default function AgentRegistration() {
  ...
  useEffect(() => {
    localStorage.setItem('intelera_agent_app_normal', JSON.stringify({ formData, files, fileNames }));
  ...
  if (submitted) {
    return (
      <div className="pt-28 bg-slate-50 min-h-screen flex items-center justify-center px-4">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white p-12 rounded-2xl shadow-sm border border-slate-200 max-w-lg text-center">
          ...
          <h2 className="text-3xl font-bold text-slate-900 mb-4 font-sans">Application Received</h2>
          <p className="text-slate-600 mb-8 leading-relaxed">Thank you, {formData.fullName}. Our partnership team will carefully review your application and contact you within 2-3 business days.</p>
            ...
            <div className="p-8">
              ...
              <div className="grid md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea required rows={4} name="personalStatement" value={formData.personalStatement} onChange={updateForm} className="w-full bg-slate-50 border border-slate-200 text-slate-900 px-4 py-2.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-colors resize-none" placeholder="Enter your personal statement or motivation..."></textarea>
                </div>
              </div>
            </div>
