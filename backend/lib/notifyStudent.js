  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:32px auto;background:#0A192F;border-radius:16px;overflow:hidden;border:1px solid rgba(100,255,218,.12);">
    ...
    <tr>
      <td style="padding:36px 40px 0;">
        <h2 style="margin:0 0 12px;color:#ccd6f6;font-size:22px;font-weight:600;">Welcome aboard, ${name || 'Student'}! 🎓</h2>
    ...
    <tr>
      <td style="padding:28px 40px 0;">
        <table width="100%" cellpadding="0" cellspacing="0" style="background:rgba(100,255,218,.05);border:1px solid rgba(100,255,218,.2);border-radius:12px;">
    ...
    <tr>
      <td style="padding:24px 40px 0;text-align:center;">
        <a href="${activationUrl}" style="display:inline-block;background:#64FFDA;color:#0A192F;text-decoration:none;font-weight:700;font-size:15px;padding:14px 36px;border-radius:8px;">
    ...
    <tr>
      <td style="padding:20px 40px 0;text-align:center;">
        <a href="${portalUrl}" style="display:inline-block;border:1px solid rgba(100,255,218,.35);color:#64FFDA;text-decoration:none;font-weight:600;font-size:14px;padding:12px 32px;border-radius:8px;background:rgba(100,255,218,.06);">
    ...
    <tr>
      <td style="padding:28px 40px 0;">
        <p style="margin:0;color:#8892b0;font-size:13px;line-height:1.65;">
          Questions? Contact us at <a href="mailto:support@anmel.com" style="color:#64FFDA;text-decoration:none;">support@anmel.com</a>.
        </p>
      </td>
    </tr>
    <tr>
...
function buildPlainText({ name, email, tempPassword, courseSlug, activationUrl }) {
  ...
  return [
    ...
    activationUrl ? `\nActivate account (link expires 72h):\n${activationUrl}` : null,
...
export async function notifyStudentProvisioned({ name, email, phone, tempPassword, courseSlug, activationUrl }) {
  ...
  try {
    ...
    if (phone && sid && token && from) {
      const body  = `Hi ${name || 'Student'}, your Anmel LMS account is ready. Email: ${email}${tempPassword ? ` | Temp pw: ${tempPassword}` : ''}. Log in at the student portal.`;
      ...
      if (!r.ok) throw new Error(`SMS failed: ${await r.text()}`);
      smsResult = { sent: true, dryRun: false };
    } else {
      smsResult = { sent: false, dryRun: true };
    }
  } catch (err) {
